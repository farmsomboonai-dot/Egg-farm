...
function openingForDay(date, prodByDate) {
  const dates = Object.keys(prodByDate || {}).sort();
  const idx = dates.indexOf(date);
  if (idx <= 0) return STOCK_OPENING;                       // วันแรก → ยกมา 2/7
  const prev = dates[idx - 1];
  if (STOCK_REMAIN_BY_DATE[prev]) return STOCK_REMAIN_BY_DATE[prev];   // วันก่อนปิดยอดแล้ว → ใช้คงเหลือจริง
  const po = openingForDay(prev, prodByDate);                // วันก่อนยังไม่ปิด → ยกมา + ผลิต (ยังไม่หักขายอดีต)
...
export default function App() {
  ...
  const salesTotals = useMemo(() => {   // ยอดขายรวมต่อสินค้า (session = ขายของวันปัจจุบัน)
    const m = {};
    Object.entries(salesLog).forEach(([pid, perCust]) => { m[pid] = Object.values(perCust).reduce((s, q) => s + (q || 0), 0); });
    return m;
  }, [salesLog]);
  const stock = useMemo(() => {   // สต็อกขายได้ = ยกมา(เมื่อวานคงเหลือ) + ผลิตวันนี้(สด) − ขายแล้ว
    const opening = openingForDay(stockDay, productionByDate);
    ...
    return s;
  }, [productionByDate, salesTotals, stockDay]);
  ...
  const recordPayment = (billNo, amount, method, slip) =>
    setPayments((prev) => ({ ...prev, [billNo]: { paid: amount, date: new Date().toLocaleDateString("th-TH"), method, slip: slip || null } }));
  ...
  return (
    <div style={S.app}>
      ...
      {view === "sales" && <SalesView stock={stock} recordSale={recordSale} addBill={addBill} bills={bills} payments={payments} trayStock={trayStock} setTrayStock={setTrayStock} trayRecords={trayRecords} drafts={drafts} setDrafts={setDrafts} />}
      ...
      {view === "stock" && <StockView salesLog={salesLog} productionByDate={productionByDate} defaultDay={stockDay} />}
...
============================================================ */
function SalesView({ stock, recordSale, addBill, bills, payments, trayStock, setTrayStock, trayRecords = [], drafts = [], setDrafts }) {
  ...
  const confirmBill = () => {
    recordSale(cartItems, customerId);   // สต็อกลดอัตโนมัติ (ดึงสดจาก salesLog) — ไม่ต้องหัก state เอง
    const bill = {
...
============================================================ */
function StockView({ salesLog = {}, productionByDate = {}, defaultDay }) {
  ...
  const reconciled = !!STOCK_REMAIN_BY_DATE[day];   // วันปิดยอดแล้ว (มีนับจริง) → คงเหลือใช้ค่านับจริง, ขาย back-calc
  ...
  const opening = openingForDay(day, productionByDate);                 // ยกมา = คงเหลือจริงของเมื่อวาน (rolling)
  const production = useMemo(() => productionToStock(productionByDate[day] || []), [productionByDate, day]);  // รับเข้า = ผลผลิตวันนั้น (สด)
  const physical = STOCK_REMAIN_BY_DATE[day];

  const rows = STOCK_ORDER.map((pid) => {
    const op = opening[pid] || 0;
    const rec = production[pid] || 0;
    let sold, remain, perCust;
    if (reconciled) { remain = physical[pid] || 0; sold = op + rec - remain; perCust = {}; }   // นับจริง → ขาย = ยกมา+รับเข้า−คงเหลือ
    else { perCust = salesLog[pid] || {}; sold = Object.values(perCust).reduce((s, q) => s + (q || 0), 0); remain = op + rec - sold; }
    return { pid, name: PRODUCT_BY_ID[pid]?.name || pid, opening: op, received: rec, total: op + rec, perCust, sold, remain };
  });
    ...
    <div style={S.wide}>
      <div style={S.subBar}>
        <span style={S.subBarTitle}>รายงานคลังไข่ประจำวัน · {dayTH}
          {reconciled ? <span style={{ fontSize: 12, fontWeight: 600, color: "#15803D" }}> · ปิดยอด (นับจริง)</span> : <span style={{ fontSize: 12, fontWeight: 600, color: "#1D4ED8" }}> · วันปัจจุบัน</span>}
        </span>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <ThaiDateField value={day} onChange={setDay} style={{ width: 190, padding: "7px 10px", borderColor: ACCENT, fontSize: 13.5 }} />
        ...
        <table style={S.table}>
          <thead>
            <tr>
              ...
              <th style={{ ...S.th, background: "#15803D", color: "#fff" }}>คงเหลือ<br />(17:00)</th>
      ...
      <div style={S.hint}>
        {reconciled
          ? <span>วันนี้ <b>ปิดยอดแล้ว</b> — คงเหลือ = ยอดนับจริง · ขายรวม = ยกมา + รับเข้า − คงเหลือ (คำนวณกลับ) · ยกมา = คงเหลือของเมื่อวาน</span>
          : <span>ยกมา = คงเหลือจริงของเมื่อวาน · รับเข้า = ผลผลิตวันนี้ (อัตโนมัติจากหน้า "ผลผลิต") · ขายรวม = ยอดขายจริงที่บันทึก · <b style={{ color: "#15803D" }}>คงเหลือ (17:00) = ยกมา + รับเข้า − ขาย</b> (กลายเป็น "ยกมา" ของพรุ่งนี้) · <b style={{ color: "#1D4ED8" }}>ประมาณการพรุ่งนี้ = คงเหลือ + รับเข้า</b></span>}
      </div>
    </div>
  );
}
